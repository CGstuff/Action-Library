...
class AddonInstallerService:
    ...
    @staticmethod
    def _subprocess_kwargs(timeout: int = 30) -> dict:
        """Common subprocess kwargs for running Blender in windowless apps.

        Suppresses console window flash on Windows and guards against
        stdin/stdout inheritance issues when there is no parent console
        (e.g. PyInstaller windowed mode).
        """
        kwargs = dict(
            capture_output=True,
            text=True,
    ...
    def verify_blender_executable(self, blender_path: str) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            result = subprocess.run(
                ...
                **self._subprocess_kwargs(timeout=10)
            )

            if result.stdout is None:
                logger.warning("Blender --version returned None stdout (windowless app?)")
    ...
    def get_blender_addons_directory(self, blender_path: str) -> Optional[Path]:
        ...
        try:
            ...
            result = subprocess.run(
                ...
                **self._subprocess_kwargs(timeout=30)
            )
    ...
    def install_addon(self, blender_path: str, storage_path: str = None, exe_path: str = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            process = subprocess.run(
                cmd,
                **self._subprocess_kwargs(timeout=60)
            )
            ...
            if process.stdout is None:
                logger.warning("Blender install returned None stdout (windowless app?)")
            logger.info(f"Blender Install Output (stdout): {stdout}")
            if stderr:
                logger.warning(f"Blender Install Output (stderr): {stderr}")

            # Check success based on output or exit code
            if process.returncode == 0 and "Action Library installed and enabled successfully" in stdout:
                ...
                if error_lines:
                    ...
                    error_msg = ("Could not read Blender output. "
                                 "Try running Action Library from a terminal to see full output.")
                else:
                    error_msg = "Unknown error during installation"
